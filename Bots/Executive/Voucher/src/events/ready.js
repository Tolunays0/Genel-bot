...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  setInterval(() => { IsimKontrol(); }, 3 * 1000);
  ...
  async function gorevKontrol() {
    ...
    async function IsimKontrol(member) {
      const tag = conf.tag || ""; 
      if (!member || !member.roles) return;
  
      const ekipRoluVarMi = member.roles.cache.has(conf.ekipRolu);
      
      if (tag && ekipRoluVarMi) {
          const newNickname = `${tag} ${member.displayName}`;
          if (member.manageable) await member.setNickname(newNickname);
      } else {
...
async function TagAlıncaKontrol() { // Tag alınca tarama
  const guild = client.guilds.cache.get(allah.GuildID);
  const tagArray = Array.isArray(tagSystem.tags) ? tagSystem.tags : [];
